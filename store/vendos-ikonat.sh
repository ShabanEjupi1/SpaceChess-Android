...
# Ekzekutohet pas `vizato.mjs`, që i vizaton nga `linux-install/spacechess/store/ikona.html`.
#
#   sh store/vendos-ikonat.sh store/ikona-gen
#
# 🚨 Burimi i ikonës jeton te depoja e LOJËS (`linux-install/spacechess/store/`),
# jo këtu: ikona e aplikacionit dhe stema e faqes duhet të mbeten i njëjti send,
# dhe dy kopje të një SVG-je ndahen që të parën herë që dikush ndreq njërën.
set -eu

G="${1:?jep dosjen me PNG-të e vizatuara}"
R="$(cd "$(dirname "$0")/.." && pwd)"
A="$R/spacechess_app/android/app/src/main/res"
W="$R/spacechess_app/web"
